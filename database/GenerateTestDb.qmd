...
training <- data.frame(
  ...
  training_expire = sample(c(NA, Sys.Date()), 50, replace = TRUE, prob = c(0.95, 0.05))
) |> 
...
## Populate `incident` and `response``
```{r}
dispatch_codes <- c(
...
incident <- data.frame(
  ...
  dispatch_code = sample(dispatch_codes, 50, TRUE),
  ...
  finalized = sample(0:1, 5, replace = TRUE, prob = c(0.05, 0.95)),
  incident_expire = sample(c(NA, Sys.Date()), 50, replace = TRUE, prob = c(0.95, 0.05))
) |> 
  mutate(incident_end = incident_start + sample(300:36000, 50, replace = TRUE),
         .after = "incident_start") |> 
  mutate(finalized = if_else(incident_end > Sys.time() - 60*60*24*7, 0, 1),
...
response <- data.frame(
  ...
  mutate(
    ...
    response_expire = sample(c(NA, Sys.Date()), 60, replace = TRUE, prob = c(0.95, 0.05))
  ) |> 
  select(id, incident_id, response_start, response_end, notes, response_expire)
...
unit <- data.frame(
  ...
  unit_expire = c(rep(NA, 3), "2025-01-01")
)
...
# loading_firefighter_response$firefighter_incident_id <- 1:nrow(loading_firefighter_incident)
